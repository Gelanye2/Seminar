...
train_all_clean <- readRDS("data/data_all_spatial.rds") %>%
...
train_model <- train_all_clean %>%
  select(-longitude, -latitude, -lga, -ward, -subvillage,-district_code, -region,-dataset,-year_recorded, -month_recorded
         ,-region_code,-population_500,location_cluster2)
#find missing values
...
saveRDS(acc_learner, file = "/media/external/s25_5/result/imp_learner_sp.rds")
...
saveRDS(result2, file = "/media/external/s25_5/result/imp_median_mode_sp.rds")
...
saveRDS(result3, file = "/media/external/s25_5/result/imp_mean_mode_sp.rds")
...
saveRDS(result4, file = "/media/external/s25_5/result/imp_delete_sp.rds")
