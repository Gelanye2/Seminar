...
graph <- po_char2fac %>>% po("imputemedian") %>>% po("encode", method = "treatment")%>>% lrn("classif.xgboost", predict_type = "prob",nthread = 7)
...
cat("ACC XG: ", round(acc, 4), "\n")
cat("BACC XG:", round(bacc, 4), "\n")
