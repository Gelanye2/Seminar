...
graph <- po_char2fac %>>% po("imputemedian") %>>% po("encode", method = "treatment")%>>% lrn("classif.lightgbm", predict_type = "prob",  objective = "multiclass", num_threads = 7)
...
cat("ACC L: ", round(acc, 4), "\n")
cat("BACC L:", round(bacc, 4), "\n")
